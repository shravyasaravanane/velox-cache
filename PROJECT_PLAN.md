...
- [x] **M7.7** Screen — **Cluster**, added to `velox-dashboard` as a sixth tab. `ClusterProxyController` (new, velox-server): a thin proxy, not a shared dependency -- `velox-cluster` stays Spring-free and JSON-library-free, and the browser never makes a cross-origin call to a different port (`ClusterNode`'s bare `HttpServer` sets no CORS headers). Parses `ClusterNode`'s tab-separated `/cluster/nodes` into JSON, and proxies kill/demo-write/demo-read the same way. Reports `reachable: false` plainly when no cluster is running, rather than surfacing a raw connection error -- `velox-cluster` is a separate process family this demo does not start. The screen itself: a node grid (health-colored cards, a kill button per healthy node) plus a "write, kill, read again" panel that proves survivability directly from the browser. **Verified through the actual UI-facing path**, not just direct `curl` to `ClusterNode`: wrote a key via `POST /api/cluster/demo-put` (the dashboard's own endpoint) through the Vite proxy, killed a node via `POST /api/cluster/kill` (the same call the kill button makes), confirmed the cluster view correctly flipped that node to `healthy: false` within ~3s, confirmed the key was still readable via `GET /api/cluster/demo-get` afterward, and confirmed the killed node's real OS process had actually exited.
- [x] ✅ **Checkpoint: "3-5 nodes, kill one live, watch it heal"** -- done twice. Automated: `ClusterNodeIntegrationTest` starts real `ClusterNode`s on ephemeral ports and proves routing, exact-R=2 replication, read-fallback-on-primary-death, and ring repair-on-recovery, all over real HTTP. Live: 3 **separate OS processes** (`java -cp ... ClusterNode node-A 127.0.0.1 7001`, etc.), joined over real cross-process HTTP calls, a real write/read round trip confirmed through every node, replication confirmed landed on exactly 2 of 3 nodes via `/internal/kv/*`, then node-A's real PID killed with `Stop-Process -Force` (a genuine hard kill, not a graceful call) -- the survivors detected it within ~2s, `demo-key` stayed readable via the replica, and a brand-new write after the crash succeeded entirely among the two survivors.
  - **A real bug the live-process demo caught that the automated tests could not**: `POST /cluster/shutdown` returned 204 but the two "stopped" node processes kept running. `HttpServer#stop()` does not shut down a custom executor -- that is the caller's job, per its own Javadoc -- and `Executors.newCachedThreadPool()`'s worker threads are non-daemon, so the JVM never exited even though the server itself had stopped. Invisible to `ClusterNodeIntegrationTest`, whose nodes all share the *test's own* JVM, which has plenty of other reasons to stay alive regardless of any one node's executor. Fixed by holding the executor and calling `shutdownNow()` in `stop()`; reproduced the failure and then the fix live, against a real separate process, not just re-run in-JVM. **Re-verified end to end through the actual dashboard UI's own endpoints** once M7.7 landed: write via `/api/cluster/demo-put`, kill via `/api/cluster/kill` (the same call the Cluster screen's kill button makes), the view correctly flipping to `healthy: false`, the key still readable via `/api/cluster/demo-get`, and the killed process confirmed actually gone.
</details>

<details>
<summary><b>Tier 8 — Durability & Polish</b></summary>

- [x] **M8.1** `WriteAheadLog` (new `com.velox.core.durability` package): length-prefixed binary records (`[4-byte length][payload][4-byte CRC32C]`), CRC32C (RFC 3720 Castagnoli, hardware-accelerated in the JDK since 9 -- not the weaker classic CRC32) over every payload, and the same three fsync policies Redis's AOF made well-known: `ALWAYS` (fsync every append), `EVERYSEC` (a background thread fsyncs at most once/second, bounded loss window), `NEVER` (no explicit fsync; `close()` still syncs, so only a real crash, not a clean shutdown, can lose anything).
- [x] **M8.2** `Snapshotter` + `Compactor` + `RecoveryManager`, tied together by the new façade `DurableStore` -- a durable KV store in its own right, deliberately **not** durability bolted onto `Cache<K,V>` (a cache is allowed to forget by design; durability is close to the opposite promise -- see `DurableStore`'s own class Javadoc for the full argument). Snapshot on compaction, WAL truncated to empty immediately after (redundant with what the snapshot now captures), `RecoveryManager` loads the latest snapshot then replays the WAL tail on top, re-checking each `Put`'s TTL against the current clock so an entry that expired while the process was down is never resurrected.
- [x] **M8.3** Crash-recovery test. A literal `kill -9` mid-fsync is not deterministic enough for an automated test, so (the same technique SQLite's and LevelDB's own crash tests use) the test directly produces the exact physical artifact a crash leaves behind: a WAL ending in a length prefix with no complete, checksummed payload behind it. Recovery must not throw, must recover everything before the tear, must discard exactly the torn record, and must physically truncate the file back to the last valid byte (not just skip the garbage in memory) so a later append never lands after dead bytes.
  - **A real memory-scaling bug found live, not in a unit test**: `WalReader` originally collected every decoded record into a `List<WalRecord>` before handing them to `RecoveryManager` to fold in -- fine at demo scale, wrong in general, since it meant replay's peak memory was the count of every operation since the last compaction, not the size of the final state (normally similar, not the same thing). Caught by an actual live demo: a real standalone writer process run with compaction deliberately disabled, killed with `Stop-Process -Force` after writing 13.1 million records (~1.5 GB), threw `OutOfMemoryError` on recovery with a 2 GB heap -- well before the far smaller final map was ever built. Fixed by changing `WalReader.read` to a streaming, callback-based API (`Consumer<WalRecord>`) that folds each record into the caller's map one at a time, bounding peak memory by the state size, which is what the compaction design was always supposed to guarantee. Re-verified against the same real 1.5 GB crashed file afterward: recovered all 13,105,049 valid records correctly in ~27 seconds on a proportionate 4 GB heap, correctly reported `corruption detected=true` (the `NEVER`-fsync-policy tail that never reached disk before the kill), and the first key's value matched exactly.
