...
- [ ] *(M6.3 remains (Internals Visualizer) -- the one that genuinely needs new introspection APIs on velox-core's eviction policies themselves (exposing LRU's list order, ARC's four lanes, the TinyLFU duel), a real backend design decision. Tier 6 checkpoint still deferred until you decide whether to take this last one on.)*
